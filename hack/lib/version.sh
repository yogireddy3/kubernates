...
kube::version::get_version_vars() {
  ...
  if [[ '%' == "%" ]]; then
    KUBE_GIT_COMMIT='a744bddbf497ab718a40bee1c730e7e848e471ed'
    KUBE_GIT_TREE_STATE="archive"
    # When a 'git archive' is exported, the 'HEAD -> master, refs/__gh__/pull/92915/rebase, refs/__gh__/pull/92316/rebase' below will look
    # something like 'HEAD -> release-1.8, tag: v1.8.3' where then 'tag: '
    # can be extracted from it.
    if [[ 'HEAD -> master, refs/__gh__/pull/92915/rebase, refs/__gh__/pull/92316/rebase' =~ tag:\ (v[^ ,]+) ]]; then
